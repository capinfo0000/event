...
### 残課題（運用・要判断）
- **CAPTCHA 未導入**: レート制限で代替。さらなる濫用が見られる場合は reCAPTCHA/hCaptcha 等の導入を検討。
- **共有 Stripe キーの構造（§3-1）**: 物理分離が必要なら Stripe Connect 化、または `ALLOW_SIGNUP=0` で単独運営へ。実装と規約・プライバシーポリシーの整合を取ること。
- **CSP の `unsafe-inline`**: 既存のインライン script/style/イベントハンドラを壊さないため許可中。将来的に nonce 化すると XSS 耐性が向上。
