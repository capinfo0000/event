...
### 残課題（運用・要判断）
- **共有 Stripe キーの構造（§3-1）**: 物理分離が必要なら Stripe Connect 化、または `ALLOW_SIGNUP=0` で単独運営へ。実装と規約・プライバシーポリシーの整合を取ること。**※方針はオーナー判断のため別途確認中。**
- **CSP の `unsafe-inline`**: 既存のインライン script/style/イベントハンドラを壊さないため許可中。完全撤廃には全ページのインラインJS/ハンドラ/styleを外部化する大規模リファクタが必要。
