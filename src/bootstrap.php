...
function default_policy_text(string $field): string
{
    switch ($field) {
        case 'cancel_policy':
            return <<<'TXT'
本イベントは事前決済（前払い）制です。お支払い後のキャンセルについては、以下の返金規定を適用します。
...
※ 返金は Stripe を通じて、お支払いに使用されたカードへ行います。決済手数料は返金されません。
...
第8条（免責）
1. 主催者と参加者との間の取引、イベントの開催・内容・中止・変更に関する一切の責任は当該当事者が負うものとし、当社はこれに関与しません。
2. 当社は、本サービスに事実上または法律上の瑕疵がないことを明示的にも黙示的にも保証しません。
3. 当社は、本サービスの中断・障害・データの消失、決済代行事業者に起因する事象等により利用者に生じた損害について、当社の故意または重過失による場合を除き責任を負いません。
4. 消費者契約法その他の強行法規が適用される場合、当社の責任は法令上許容される範囲に限定されます。

第9条（個人情報の取扱い）
当社は、本サービスの利用により取得する個人情報を、別途定めるプライバシーポリシーに従って適切に取り扱います。

第10条（準拠法・裁判管轄）
本規約の準拠法は日本法とし、本サービスに関して当社と利用者との間に紛争が生じた場合には、当社の所在地を管轄する裁判所を第一審の専属的合意管轄裁判所とします。
...
4. 安全管理措置
当社は、個人情報の漏えい・滅失・毀損を防止するため、通信の暗号化（HTTPS）、保管情報の暗号化、アクセス制限、二段階認証等の適切な安全管理措置を講じます。参加者名簿は主に決済事業者（Stripe）側で管理され、当社はアカウント・イベント情報を適切に管理します。
